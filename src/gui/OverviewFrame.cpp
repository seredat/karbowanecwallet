...
OverviewFrame::OverviewFrame(QWidget* _parent) : QFrame(_parent), m_ui(new Ui::OverviewFrame), m_transactionModel(new RecentTransactionsModel) {
  ...
  m_ui->m_tickerLabel7->setText(CurrencyAdapter::instance().getCurrencyTicker().toUpper());
...
void OverviewFrame::actualDepositBalanceUpdated(quint64 _balance) {
  ...
  m_ui->m_totalDepositLabel->setText(CurrencyAdapter::instance().formatAmount(_balance + pendingDepositBalance).remove(','));
}
...
void OverviewFrame::pendingDepositBalanceUpdated(quint64 _balance) {
  ...
  m_ui->m_totalDepositLabel->setText(CurrencyAdapter::instance().formatAmount(_balance + actualDepositBalance).remove(','));
}
